  ...
  return (
    <div className="story-chart-beforeafter">
      ...
      <div className="story-chart-ba-row">
        ...
        <span className={`story-chart-ba-bar is-after ${activated ? 'is-grown' : ''}`} style={{ width: activated ? `${toWidth}%` : '0%' }} />
      </div>
      <span className="story-chart-label">{metric.shortLabel?.trim() || metric.label}</span>
    </div>
  )
    ...
    <div className="story-chart-trend">
      <svg viewBox="0 0 160 72" preserveAspectRatio="none" aria-hidden="true">
        ...
        <path
          ...
          strokeLinecap="round"
        />
        <circle cx="6" cy="62" r="3.5" fill="#FF5F6D" opacity={activated ? 1 : 0} style={{ transition: 'opacity 300ms ease' }} />
        <circle cx="154" cy="8" r="3.5" fill="#FF2E88" opacity={activated ? 1 : 0} style={{ transition: 'opacity 300ms ease 700ms' }} />
      </svg>
...
export function StoryMetricChart({ metric, activated }: { metric: CaseStudyMetric; activated: boolean }) {
  switch (metric.display) {
    case 'beforeafter':
      return <BeforeAfterTile metric={metric} activated={activated} />
    case 'donut':
      return <DonutTile metric={metric} activated={activated} />
    case 'bars':
      return <HBarCompareTile metric={metric} activated={activated} />
    case 'trend':
      return <TrendLineTile metric={metric} activated={activated} />
    default:
      return <BigStatTile metric={metric} activated={activated} />
  }
}
